...
transformer_model = Bert_Model_classifier(**model_params)
...
transformer_model.fc.register_forward_hook(get_activation('fc_layer'))
...
fc_repr=np.zeros((csv_data.shape[0],1024))
...
for i in tqdm(np.arange(csv_data.shape[0])):
    ...
    fc_representation=activation['fc_layer'].squeeze(0).cpu().numpy()
