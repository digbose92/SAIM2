...
def main(config_data):
    ...
    #dict_label=dict()
    # for i,top in enumerate(topic_class_label):
    #     top=top.strip()
    #     dict_label[top]=i
    ...
    model=Bert_Model_classifier(**model_params)
    print(model)
    ...
    for epoch in range(1, max_epochs+1):
        ...
        for id,(dict_data) in enumerate(tqdm(train_dl)):
            ...
            output,hidden_state=model(input_ids, attention_mask, token_type_ids)
            train_logits_temp=log_softmax(output).to('cpu')
        ...
        valid_accuracy,f1_score_val,precision_score_val,recall_score_val,val_loss=gen_eval_score(valid_dl,model,device,criterion)
        ...
        model.train(True)
        #save checkpoint for every model 
        #torch.save(model, os.path.join(sub_folder_model,timestr+'_'+config_data['model']['model_type']+'_epoch_'+str(epoch)+'_model.pt'))
...
if __name__ == '__main__':
    ...
    #print(type(config_data['transforms']['resample_method']))
    main(config_data)
